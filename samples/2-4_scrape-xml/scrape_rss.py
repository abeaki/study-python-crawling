...
# getroot()メソッドでXMLのルート要素（この例では<rss>要素）に対応するElementオブジェクトを得る。
root = tree.getroot()

# findall()メソッドでXPathにマッチする要素のリストを取得する。
# channel/item は<channel>要素の子要素である<item>要素を表す。
for item in root.findall('channel/item'):
    # find()メソッドでXPathにマッチする要素を取得し、text属性で要素の文字列を取得する。
    title = item.find('title').text  # <title>要素の文字列を取得する。
    url = item.find('link').text     # <link>要素の文字列を取得する。
